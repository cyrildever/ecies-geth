...
describe('ecies', () => {
    describe('kdf', () => {
        ...
        it('should round the ouput length to the next 32 mutiple', () => {
            const secret = Buffer.from('b9fc3b425d6c1745b9c963c97e6e1d4c1db7a093a36e0cf7c0bf85dc1130b8a0', 'hex')
            const found1 = ecies.kdf(secret, 35)
            const found2 = ecies.kdf(secret, 64)

            return found1.should.eqls(found2)
        })
        it('should return an empty buffer for optoutLength = 0', () => {
            const secret = Buffer.from('b9fc3b425d6c1745b9c963c97e6e1d4c1db7a093a36e0cf7c0bf85dc1130b8a0', 'hex')
            const found = ecies.kdf(secret, 0)
